...
class DocumentProcessor:
    ...
    def ingest_documents(self):
        ...
        uploaded_files = st.file_uploader(
            #####################################
            # Allow only type `pdf` -- done
            # Allow multiple PDFs for ingestion --accept multiple file
            #####################################
            label="Upload PDF files", #from error got information to add  
        ...
        if uploaded_files is not None:
            for uploaded_file in uploaded_files:
                ...
                #####################################
                # Use PyPDFLoader here to load the PDF and extract pages. --done( extract_pages = pdf_loader.load();)
                # https://python.langchain.com/docs/modules/data_connection/document_loaders/pdf#using-pypdf
                # You will need to figure out how to use PyPDFLoader to process the temporary file. --doene(  pdf_loader = PyPDFLoader(temp_file_path))
                pdf_loader = PyPDFLoader(temp_file_path)
                extract_pages = pdf_loader.load();
                
                # Step 3: Then, Add the extracted pages to the 'pages' list. --done(self.pages.extend(extract_pages))
                #####################################
                self.pages.extend(extract_pages)
